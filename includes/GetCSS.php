...
class GetCSS{
	...
	static function getIconCSS( $icon, $isSize = true, $isColor = true ) {
		...
		$styles = (!$fontSize || !$isSize ? '' : 'font-size: '. $fontSize .'px;') . ($isColor ? $colorCSS : '');

		return $styles;
	}

	static function getSeparatorCSS( $separator ) {
		extract( $separator );
		$width = $width ?? '50%';
		$height = $height ?? '2px';
		$style = $style ?? 'solid';
		...
		return "width: $width;" . (('0' === preg_replace('/[^\d]/', '', $height)) ? '' : "border-top: $height $style $color;");
	}
	...
	static function getShadowCSS( $shadow, $type = 'box' ) {
		extract( $shadow );
		$hOffset = $hOffset ?? '0px';
		$vOffset = $vOffset ?? '0px';
		$blur = $blur ?? '0px';
