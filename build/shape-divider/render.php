...
// Generate Styles
if( !function_exists( 'bBlocksShapeDividerStyle' ) ) {
	function bBlocksShapeDividerStyle( $attributes, $id, $prefix){
		extract( $attributes );

		$mainSl = "#$id";

		$transform = 'top' === $shapePosition ? '0deg' : '180deg';

		$shapeDividerStyle = new BBlocksStyleGenerator();
		$shapeDividerStyle::addStyle( "$mainSl", [
			'min-height' => $height['desktop'],
			GetCSS::getBackgroundCSS( $background ) => '',
			'padding' => implode(' ', $padding)
		] );
		$shapeDividerStyle::addStyle( "$mainSl svg", [
			'width' => "calc($shapeWidth% + 1.3px)",
			'height' => $shapeHeight['desktop'],
			'fill' => $shapeColor,
			'top' => 'top' === $shapePosition ? '0' : 'auto',
			'bottom' => 'bottom' === $shapePosition ? '0' : 'auto',
			'transform' => "rotate( $transform )"
		] );

		return $shapeDividerStyle::renderStyle();
	}
}

if( !function_exists( 'bBlocksShapePath' ) ){
	function bBlocksShapePath( $shape ){
		$path = '';
